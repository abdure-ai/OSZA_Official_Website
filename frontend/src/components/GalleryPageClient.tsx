...
import { fetchGalleryCategories, fetchGallery, GalleryCategory, GalleryItem } from '@/lib/api';
import GallerySlideshow from '@/components/GallerySlideshow';
import { FaImages } from 'react-icons/fa';

const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000';

// Server component data is fetched via a separate async wrapper; this is the client component
// that handles the interactive state (slideshow).
...
export default function GalleryPageClient({ categories }: Props) {
    ...
    const openAlbum = useCallback(async (category: string) => {
        ...
        const images = await fetchGallery(undefined, category);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {categories.map(cat => (
                    <button
                        ...
                        className="group relative rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 aspect-[4/3] bg-gray-200 text-left focus:outline-none focus:ring-2 focus:ring-green-500"
                    >
                        {cat.cover_url ? (
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-4">
                            <p className="text-white font-bold text-lg leading-tight">{cat.category}</p>
